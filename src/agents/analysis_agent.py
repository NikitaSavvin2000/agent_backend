...
async def analytics_agent(user_task: str, full_describe_data: str, df):
    ...
    - выполняет код,
    - забирает html_output / result_analysis / df_result / png_base64 из локальных переменных,
    ...
    try:
        ...
        if response is None:
            ...
            return (
                None,
                None,
                "Не удалось получить ответ от модели при генерации анализа.",
                None,
            )
        ...
        try:
            ...
            return (
                None,
                None,
                "Не удалось разобрать ответ модели при генерации анализа.",
                None,
            )
        ...
        png_base64 = local_vars.get("png_base64")  # на случай, если модель вернёт картинку
                ...
                if isinstance(result_analysis, str):
                    ...
                    try:
                        base_text = json.dumps(
                            result_analysis, ensure_ascii=False, indent=2
                        )
                    except Exception:
                        base_text = str(result_analysis)

                # здесь уже ВСЁ строки — ошибки сложения dict + str не будет
                result_analysis = (
                    base_text + "\n\n## **Обоснование**\n" + result_resonating
                )

        # Всегда возвращаем 4 значения, чтобы сверху не было ошибок распаковки
